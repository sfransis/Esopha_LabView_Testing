...
class DilatedParllelResidualBlockB1(nn.Module):
    def __init__(self, nIn, nOut, drop_flag, prob=0.03):
        super().__init__()
        n = int(nOut / 4)
        n1 = nOut - 3 * n
        self.c1 = C(nIn, n, 3, 1)
...
class PSPDec(nn.Module):
    def __init__(self, nIn, nOut, downSize, upSize=48):
        ...
        self.features = nn.Sequential(
            ...
            nn.Upsample(size=upSize, mode='bilinear', align_corners=True)
        )
...
class ResNetC1(nn.Module):
    ...
    def __init__(self, classes, drop_flag, prob):
        super().__init__()
        self.level1 = CBR(3, 16, 7, 2)

        self.p01 = PSPDec(16 + classes, classes, 160, 192) #, 192
        ...
        self.level2_0 = DilatedParllelResidualBlockB1(128, 128, drop_flag)
        self.level2_1 = DilatedParllelResidualBlockB1(128, 128, drop_flag)

        self.p10 = PSPDec(classes + 256, 64, 80, 96) #, 96
        ...
        self.level3_1 = DilatedParllelResidualBlockB1(256, 256, drop_flag, prob)
        self.level3_2 = DilatedParllelResidualBlockB1(256, 256, drop_flag, prob)

        self.level4_1 = DilatedParllelResidualBlockB1(256, 256, drop_flag, prob)
        self.level4_2 = DilatedParllelResidualBlockB1(256, 256, drop_flag, prob)
        self.level4_3 = DilatedParllelResidualBlockB1(256, 256, drop_flag, prob)
...
class ResNetC1_YNet(nn.Module):
    ...
    def __init__(self, classes, diagClasses, prob, drop_flag, segNetFile=None):
        super().__init__()

        self.level4_0 = DownSamplerA(512, 128)
        self.level4_1 = DilatedParllelResidualBlockB1(128, 128, drop_flag, prob)
        self.level4_2 = DilatedParllelResidualBlockB1(128, 128, drop_flag, prob)

        self.br_con_4 = BR(256)

        self.level5_0 = DownSamplerA(256, 64)
        self.level5_1 = DilatedParllelResidualBlockB1(64, 64, drop_flag, prob)
        self.level5_2 = DilatedParllelResidualBlockB1(64, 64, drop_flag, prob)
        ...
        # segmentation model
        self.segNet = ResNetC1(classes, drop_flag, prob)
##        if segNetFile is not None:
##            print('Loading pre-trained segmentation model')
##            print(segNetFile)
##            own_state = self.segNet.state_dict()
##            state_dict = torch.load(segNetFile, map_location='cuda:0')
...
##            self.segNet.load_state_dict(own_state)
